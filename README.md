...
* 🇻🇳 **Đóng góp chính:**
    ...
    1.  🇻🇳 Xây dựng và mô tả công khai một **tập dữ liệu lớn, đa dạng** gồm các file `README.md`, bao gồm hơn **năm mươi chủ đề** khác nhau.
    ...
    2.  🇻🇳 Phát triển quy trình **tiền xử lý văn bản mạnh mẽ** được điều chỉnh cho tài liệu phần mềm (ví dụ: làm giàu stopword bằng các mã thông báo dành riêng cho miền).
    ...
    3.  🇻🇳 Áp dụng và đánh giá các mô hình dựa trên Transformer hiện đại, tận dụng **PEFT (LoRA)** để đạt được hiệu suất cao và hiệu quả tính toán.
...
* 🇻🇳 **Chiến lược lấy mẫu (để đảm bảo tính đa dạng):** Đối với mỗi chủ đề, thu thập tới 2.000 kho lưu trữ dựa trên 4 tiêu chí bổ sung (mỗi tiêu chí khoảng 500 repo):
    1.  🇬🇧 **Most Starred**
    1.  🇻🇳 **Most Starred** (Được gắn sao nhiều nhất).
    2.  🇬🇧 **Most Forked**
    2.  🇻🇳 **Most Forked** (Được fork nhiều nhất).
    3.  🇬🇧 **Recently Updated**
    3.  🇻🇳 **Recently Updated** (Được cập nhật gần đây).
    4.  🇬🇧 **Best Match** (Random sampling for diversity)
    4.  🇻🇳 **Best Match** (Mẫu ngẫu nhiên để tăng tính đa dạng).
...
1.  🇻🇳 **Loại bỏ Code và URL:** Xóa các khối mã Markdown, đoạn mã nội tuyến và siêu liên kết.
2.  🇬🇧 **Markdown Syntax Cleaning:** Normalized headers, bold/italic formatting to plain text.
2.  🇻🇳 **Làm sạch Cú pháp Markdown:** Chuẩn hóa các dấu tiêu đề, định dạng in đậm/in nghiêng thành văn bản thuần túy.
3.  🇬🇧 **Tokenization and Lemmatization:** Used `NLTK` to reduce word inflections.
3.  🇻🇳 **Tokenization và Lemmatization:** Sử dụng `NLTK` để giảm các dạng biến tố của từ.
...
4.  🇻🇳 **Lọc Stopword:** Mở rộng danh sách stopword tiếng Anh tiêu chuẩn bằng một danh sách các thuật ngữ liên quan đến lập trình (ví dụ: *install, build, repository, command, example, file*) để giảm thiểu nội dung không mang thông tin.
5.  🇬🇧 **Lowercasing and Noise Removal:** All text was lowercased and cleaned of punctuation, digits, and special characters.
5.  🇻🇳 **Chuyển về chữ thường và loại bỏ nhiễu:** Tất cả văn bản được chuyển về chữ thường và làm sạch khỏi dấu câu, chữ số và ký tự đặc biệt.

---

## 🛠️ Methodology & Models
## 🛠️ Phương pháp & Kiến trúc Mô hình
...
1.  🇻🇳 **Mô hình Học máy Cổ điển:** Huấn luyện trên các nhúng MiniLM:
    * Logistic Regression (LR)
    * Random Forest (RF)
    * Support Vector Classifier (SVC) (using a linear kernel)
    * K-Nearest Neighbors (KNN)
2.  🇬🇧 **Modern Transformer Model (PEFT):**
2.  🇻🇳 **Mô hình Transformer Hiện đại (PEFT):**
